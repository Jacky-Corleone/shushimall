...
//			}
	...
	public ExecuteResult<String> changeRegion(String ctoken, String uid, String regionId, Integer orderType,
			String buyType) {
		UserDTO user = userExportService.queryUserById(Long.valueOf(uid));
		if (StringUtils.isNotBlank(buyType) && "NOW".equals(buyType)) {
			String buyNowProducts = redisDB.get("buyNowProducts_"+user.getUname());
			if(StringUtils.isNotBlank(buyNowProducts)){
				...
				if (pros != null && pros.size() > 0) {
					...
					redisDB.addObject("buyNowProducts_"+user.getUname(), JSON.toJSONString(pros), 60 * 60 * 24);
				}
			}
		}
	...
	private ExecuteResult<TradeOrderCreateDTO> createOrder(String shopPromoId, ShopCartDTO cart, TradeOrdersDTO dto) {
		...
		if (dto.getCreatorId() != null) {
			...
			dto.setCreatorId(dto.getBuyerId());
			dto.setBuyerId(creatorId);
		}
		// 如果有服务商品，该订单就为服务订单
		...
		for (ShopOutPriceDTO shop : cart.getShops()) {
			...
			if (shop.getQuantity() > 0) {
				for (ProductInPriceDTO pro : shop.getProducts()) {
					if (pro.getChecked() && pro.getAddSource() != null && pro.getAddSource() > 3) {
						isService = true;
						break;
					}
				}
			...
			if (shop.getQuantity() > 0) {
				...
				for (ProductInPriceDTO pro : shop.getProducts()) {
					if (pro.getChecked()) {
						...
						// 设置套装商品订单记录
						List<TradeOrderItemsPackageDTO> orderItemsPackageDTOs = new ArrayList<TradeOrderItemsPackageDTO>();
						// 套装单品记录
						if(pro.getSubProducts() != null && pro.getSubProducts().size() > 0){
							for(ProductInPriceDTO subProduct : pro.getSubProducts()){
								...
								itemsPackageDTO.setSubNum(subProduct.getQuantity() * pro.getQuantity());
								itemsPackageDTO.setAddSource(ItemAddSourceEnum.NORMAL.getCode());
								orderItemsPackageDTOs.add(itemsPackageDTO);
							}
						}
						// 套装增值服务
						if(pro.getValueAddedProducts() != null && pro.getValueAddedProducts().size() > 0){
							for(ProductInPriceDTO valueAddedProduct : pro.getValueAddedProducts()){
								...
								itemsPackageDTO.setSubNum(valueAddedProduct.getQuantity() * pro.getQuantity());
								orderItemsPackageDTOs.add(itemsPackageDTO);
							}
						}
						// 套装辅料类型
						if(pro.getAuxiliaryProducts() != null && pro.getAuxiliaryProducts().size() > 0){
							for(ProductInPriceDTO auxiliaryProduct : pro.getAuxiliaryProducts()){
								TradeOrderItemsPackageDTO itemsPackageDTO = new TradeOrderItemsPackageDTO();
								itemsPackageDTO.setItemId(pro.getItemId());
								itemsPackageDTO.setSkuId(pro.getSkuId());
								itemsPackageDTO.setSubItemId(auxiliaryProduct.getItemId());
								itemsPackageDTO.setSubSkuId(auxiliaryProduct.getSkuId());
								itemsPackageDTO.setAddSource(ItemAddSourceEnum.AUXILIARYMATERIAL.getCode());
								itemsPackageDTO.setSubSkuPrice(auxiliaryProduct.getSkuPrice());
								itemsPackageDTO.setSubNum(auxiliaryProduct.getQuantity() * pro.getQuantity());
								orderItemsPackageDTOs.add(itemsPackageDTO);
							}
						}
						// 套装基础服务
						if(pro.getBasicProducts() != null && pro.getBasicProducts().size() > 0){
							for(ProductInPriceDTO basicProduct : pro.getBasicProducts()){
								TradeOrderItemsPackageDTO itemsPackageDTO = new TradeOrderItemsPackageDTO();
								itemsPackageDTO.setItemId(pro.getItemId());
								itemsPackageDTO.setSkuId(pro.getSkuId());
								itemsPackageDTO.setSubItemId(basicProduct.getItemId());
								itemsPackageDTO.setSubSkuId(basicProduct.getSkuId());
								itemsPackageDTO.setAddSource(ItemAddSourceEnum.BASICSERVICE.getCode());
								itemsPackageDTO.setSubSkuPrice(basicProduct.getSkuPrice());
								itemsPackageDTO.setSubNum(basicProduct.getQuantity() * pro.getQuantity());
								orderItemsPackageDTOs.add(itemsPackageDTO);
							}
						}
						itemDto.setTradeOrderItemsPackages(orderItemsPackageDTOs);
