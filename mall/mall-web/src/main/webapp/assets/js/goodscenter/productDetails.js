$(document).ready(function(){
	...
	$(".js_address_title").click(function(){
		$("#nTab_address_div").toggle();
		/*
		$('#nTab_address_div').mouseleave(function(){
	...
	var addSource = $("#addSource").val();//商品来源
	var itemSkuAttr = ''; // 套装商品的销售属性key:value
	//console.log("addSource="+addSource);
	//页面初始化加载
	$(".js_attr").each(function(){
		if($(this).hasClass("border-8")){
			if(addSource == 3){
				itemSkuAttr = $(this).attr("name");
			}
	...
	if(itemStatus == 4){
		// 套装商品
		if(addSource == 3 && itemSkuAttr){
			if(hasPrice == 1 ){//商品有价格时加载， 我要询价时不加载
				addCombinationCartHTML(itemSkuAttr);
		...
		if($.trim(shoucangshangpin) == "收藏商品"){
			$.ajax({
				...
				success: function(data){
					...
					$("#wjx").attr("class","demo-icons fa-star font_ffd font_14");
...
//    	$(".js_xiang_manjian").show();
//    })
	$(".js_qing_manjian").click(function(){
		$(".js_xiang_manjian").show();
    });
    $(".js_down_manjian").click(function(){
    	$(".js_xiang_manjian").hide();
    });
	...
	//判断是否存在子站
	checkServiceItemCity();
});

// 重新计算sku价格
function addValueAddedPrice(){
	if($("#skuId").val()){
		buildBuyListHTML();
	}
}
function checkServiceItemCity(){
	//判断商品是否为服务商品或者套装商品
	var cityCode = $.cookie('regionCode');
	$.ajax({
		type: "POST",
		dataType: "json",
		url: $("#contextPath").val()+"/productController/checkServiceItemCity",
		data: {
			itemId : $("#itemId").val(),
			cityCode : cityCode
		},
		success: function(data){
			if(data.result){
				...
				$("#checkService").val(data.resultMessage);
			}
		}
	});
}
...
function getPackageSku(){
	...
	var packageSkuId = $('[name=packageSkuId]').val();
	$.ajax({
...
// 查询增值服务
function getValueAddedService(){
	var packageItemId = $("#itemId").val();
	var areaCode = $("#areaCode").val();
	var valueAddedSkuIds = getValueAddedSkuIds();
	$.ajax({
		url: $("#contextPath").val()+"/productController/getValueAddedService",
		data: {
		...
		dataType: "html",
		success: function(data){
			$('#valueAddedServiceDiv').html(data);
		}
	});
}

// 获取选中的增值服务
function getValueAddedSkuIds(){
	// 增值服务
	var valueAddedSkuIds = [];
	$.each($('.serve_li .active_serve').find('input:eq(0)'), function(i, o){
		if($(o).attr('data')){
			valueAddedSkuIds.push($(o).attr('data'));
		}
	});
...
// 显示&隐藏增值服务下拉选项
function toggleService(o){
	if($(o).find('.serve_t_div').is(':hidden')){
		$(o).find('.serve_t_div').show();
	} else{
		$(o).find('.serve_t_div').hide();
	}
}
// 增值服务选中
function toggleActiveService(o){
	if($(o).parents('.serve_div').hasClass('active_serve')){
		$(o).parents('.serve_div').removeClass('active_serve');
		addedServiceChange('');
	} else{
		$(o).parents('.serve_div').addClass('active_serve');
		addedServiceChange($(o).attr('data'));
	}
}
// 增值服务下拉选中
function selectService(o){
	$(o).parents('.serve_div').addClass('active_serve');
	var oldValue = $(o).parents('.serve_div').find('input:eq(0)').attr('data');
	var newValue = $(o).attr('data');
	var oldText = $(o).parents('.serve_div').find('input:eq(0)').val();
	var newText = $(o).html();
	var oldTitle = $(o).parents('.serve_div').find('input:eq(0)').attr('title');
	var newTitle = $(o).attr('title');
	// 数据交换
	$(o).parents('.serve_div').find('input:eq(0)').attr('data', newValue);
	$(o).attr('data', oldValue);
	$(o).parents('.serve_div').find('input:eq(0)').val(newText);
	$(o).html(oldText);
	$(o).parents('.serve_div').find('input:eq(0)').attr('title', newTitle);
	$(o).attr('title', oldTitle);
	addedServiceChange(newValue);
}
			...
			success: function(data){
				...
				if(data.success){
					...
					if(res!=null && typeof(res)!="undefined"){
						...
						$("#shoujia").html('<strong class="font_fe font_fz fl font_40" style="position:relative;">暂无报价</strong>');
...
function toProOrderView(activitesDetailsId){
	// 是否为集采商品
	var isCentralPurchasing = $("#isCentralPurchasing").val();
	var itemId = $("#itemId").val();//商品Id
	var itemStatus;
	var msg;
	$.ajax({
	...
	if(isCentralPurchasing && (itemPrice==null || typeof(itemPrice)=="undefined")){
		alert("平台没有维护集采价，请联系平台");
		return;
	}
	...
	//有增值服务判断子站是否存在
	if($("#addService").val() != 'no' && $("#addService").val() != 'ok'){
		alert($("#addService").val());
		return;
	}
	
	//判断商品是否为服务商品或者套装商品 子站是否存在
	if($("#checkService").val() != 'ok'){
		alert($("#checkService").val());
		return;
	}
...
function plus(obj){
	var num = $(obj).siblings("input.number_js").val();
	if (num == '') {
		$(obj).siblings("input.number_js").val(1);
		return;
	}
	var qty = $("#qty_js").val();
	...
	*/
	...
	$(obj).siblings("input.number_js").val(number);
	if(!isCentralPurchasing){
		var hasPrice = $("#hasPrice").val();//是否有价格，有价格时再去加载价格、库存，无价格显示我要询价
		var price_name = $("#doAskPrice").attr("name");//用户咨询过价格，读取价格
		if(hasPrice==1 || (price_name!=null && typeof(price_name) != "undefined")){//商品有价格时加载， 我要询价 时不加载
			...
			if(data.priceObject.success){
				...
				if(skuPrice != null && typeof(skuPrice) != "undefined"){
					if(disPrice != null && typeof(disPrice) != "undefined"){
						...
						$("#itemPrice_js").html(barginPrice + "元");
						// 显示特价优惠
						$("#promotion_title").show();
						$("#marketPrice_div").css("margin-left","-50px");
//						$("#cuxiaojia").show();
//						$("#itemCXPrice_js").html("￥"+barginPrice);
////					$("#shoujia").html("<div class=\"mid_f_right\" id=\"itemPrice_js\"><del>￥"+skuPrice+"</del></div>");
//						$("#itemPrice_js").html(skuPrice + "元");
					} else{
						//隐藏促销价信息
//						$("#cuxiaojia").hide();
//						$("#shoujia").html('<strong class="font_fz font_40" ><span id="itemPrice_js">￥'+skuPrice+'</span></strong>');
						$("#itemPrice_js").html(skuPrice + "元");
						// 隐藏特价优惠
						$("#promotion_title").hide();
						$("#marketPrice_div").css("margin-left","10px");
					}
				} else{
//					$("#shoujia").html('<strong class="font_fz font_40" ><span id="itemPrice_js"></span></strong>');
					$("#itemPrice_js").html("");
				}
...
//						$("#promotionMsg_manjian").html(activityName);
					...
					if(promotionMarkdown != null && typeof(promotionMarkdown) != 'undefined' && promotionMarkdown.length > 0){
						...
						for(var i = 0;i < promotionMarkdown.length;i++){
							disPrice = promotionMarkdown[i].disPrice.toFixed(2);
							markdownType = promotionMarkdown[i].markdownType;
							discountPercent = promotionMarkdown[i].discountPercent;
							promotionPrice = promotionMarkdown[i].promotionPrice;
							activityName = promotionMarkdown[i].activityName;
							serverTime = promotionMarkdown[i].serverTime;
							startTime = promotionMarkdown[i].startTime;
							endTime = promotionMarkdown[i].endTime;
						}
						if(!skuId){
							// 显示直降信息
							...
							var zhijiang = '';
							if(markdownType == 1){
								$("#promotionMsg_zhijiang").html(discountPercent + "折优惠 ");
								zhijiang = discountPercent + "折优惠 ";
							} else if(markdownType == 2){
								$("#promotionMsg_zhijiang").html("直降 " + promotionPrice + " 元 ");
								zhijiang = "直降 " + promotionPrice + " 元 ";
							}
							$("#promotionList_zhijiang").append('<ul class="hei_30"><li class="wid_90 fl over_ell" title="'+activityName+'">'+activityName+'</li><li class="wid_160 fl over_ell" title="'+zhijiang+'">'+zhijiang+'</li> <li class="wid_170 fl" >剩余时间：<span countDown="true" serverTime="'+serverTime+'" startTime="'+startTime+'" noStartTip="还没有开始" endTime="'+endTime+'" endTip="已结束" callBack="">正在计算时间...</span></li></ul>');
						} else{
							$("#promotion_zhijiang").hide();
						}
						if(disPrice && skuId){
							$("#itemPrice_js").html(disPrice + "元");
							// 显示特价优惠
							$("#promotion_title").show();
							$("#marketPrice_div").css("margin-left","-50px");
						}
...
//							$("#itemPrice_js").html(skuShowPrice + "元");
//							// 显示特价优惠
//							$("#promotion_title").show();
//							$("#marketPrice_div").css("margin-left","-50px");
...
function addressSelection(addressId, code, name){
	...
	*/
		...
		$(".js_address").attr("title", $(".js_tabAddress_one").html()+name);
		$("#areaCode").val(code);
		$("#nTab_address_div").hide();
...
function addedServiceChange(value){
	if(value == ''){
		$("#addService").val('no');
	}else{
		//判断是否可以结算服务商品
		var cityCode = $.cookie('regionCode');
  		$.ajax({
			type: "POST",
			dataType: "json",
			url: $("#contextPath").val()+"/productController/checkMallTheme",
			data: {
				cityCode : cityCode
			},
			success: function(data){
				if(data.result){
					...
					$("#addService").val(data.resultMessage);
				}
			}
		});
	}
