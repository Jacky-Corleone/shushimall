...
function skuContentHtml(sellPriceDivNum,areaNum,cityAreaPriceNum,valueObj,id,skuValue,skuArrayVal,skuin){
	...
	html += "<th width=\"120\" class=\"border-4\"><font class=\"mandatory ftred\">*</font>销售价(元)</th>";
	html += "<th width=\"60\" class=\"border-4\">编辑</th>";
	html += "<th width=\"190\" class=\"border-4\">适用范围</th>";
	html += "</tr>";
	if(isAllContry || sellPrices==""){
...
//	html += "<td><input type=\"text\" onkeyup=\"numInput(this,0)\" onblur=\"calculationInventory()\" maxlength=\"7\" name=\""+skuProductId+"skuInventory\" id=\""+productNum+"_skuInventory\" value=\""+skuInventory+"\" class=\"wid_120 hei_30 border-1 mar_l10\" /> </td>";
	...
	if(isView){
		...
		$("input[type='text']").not("#keyword_searchForm")
...
function skuPicContentHtml(valueObj,picDivId,skuIdPrefix,picNum,picObj){
	...
	html +="</ul>";
	html +="<ul  class=\"mar_tb10 list_top list_top2\">";
...
function areaContentHtml(namePre,namePrefix,idPrefix,rowNum,areaPriceNum,defaultMinNum,numIndex,costPriceId,addToArray){
	...
	if(!isView){
		if(isAllContry){
			html += "<a class=\"cursor as\" onclick=\"removeElement('areaPriceTR_"+rowNum+"')\"></a>";
		}else{
    ...
    if(isAllContry == true){
    	...
    	html += "<div style=\"margin: 0px auto; width: 100%; overflow: hidden; max-height: 68px;\"><a class='cursor as' id=\"showArea_"+rowNum+"\" title='"+showAreaName+"'";
    	if(!isView){
    		html += "onclick=\"selectArea('areaId_"+rowNum+"','areaName_"+rowNum+"','showArea_"+rowNum+"',"+rowNum+")\"";
    	}else{
    		html += "onclick=\"showAreaNames(this)\"";
    	}
...
//用户点击展示不全的区域弹出文本框将未展示全的区域全部展示出来
function showAreaNames(obj){
	alert($(obj).html());
}
//检查商品编号是否已经存在
...
function showSelectArea(selectAreaIdVal,selectAreaNameVal){
	...
	if(selectAreaIdVal == "") {
		$("#"+showAreaNameIdVal).text("选择地区");
		$("#"+showAreaNameIdVal).attr("title","");
	}else{
...
//			}
			...
			html+="数量"+"<input type=\"text\" id='"+itemId+"' maxlength='4' onkeyup=\"numInput(this)\" name=\"sub_num\" value='1' class=\"input_Style2 hei_24 wid_30 mar_l10\">";
			html+="</div>";
	}
