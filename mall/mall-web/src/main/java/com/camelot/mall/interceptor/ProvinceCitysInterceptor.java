...
public class ProvinceCitysInterceptor implements HandlerInterceptor {
    ...
    public boolean preHandle(HttpServletRequest request, HttpServletResponse response, Object handler) throws Exception {
    	...
    	if(list == null){
        	...
        	List<AddressBase> provinceList = addressBaseService.queryAddressBase("0"); //获取省
    		List<AddressBaseDTO> cityList = addressBaseService.getAddressesByLevel(2); //获取市 // 为了效率， 查出全部然后分类
    		
    		for(Object obj : provinceList) {
    			AddressBaseDTO province = (AddressBaseDTO) obj;
