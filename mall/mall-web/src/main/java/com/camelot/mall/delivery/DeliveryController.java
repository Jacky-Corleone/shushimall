...
import com.camelot.mall.sellcenter.mallshop.shopInfoController;
...
public class DeliveryController {
	...
	public  Map<String,Object> getDeliveryInfo(String orderItemId, Model model) {
		...
		if(deliveryDTO!=null){
			...
			//如果运单信息为空，则提示暂无数据
			if (StringUtils.isBlank(deliveryDTO.getMessage())) {
	...
	public Map<String,Object> toAddDelivery(String orderItemId) {
	...
	public Map<String,Object> sendDeliberyMessage(SendBodyDTO sendBodyDTO, DeliveryDTO deliberyDto, Model model){
		
		Map<String,Object> returnMap = new HashMap<String,Object>();
		String code = DeliveryEnum.CODE_500.getReturnCode();//默认500
		ExecuteResult<ItemDTO> er =itemExportService.getItemById(deliberyDto.getItemId());
		ItemDTO item = null;
		if(er!=null&&er.getResult()!=null){
			item = er.getResult();
			//商品名称
			sendBodyDTO.setCommodity(item.getItemName());
			ExecuteResult<ShopDTO> shopEr = shopExportService.findShopInfoById(item.getShopId());
			if(shopEr!=null&&shopEr.getResult()!=null){
				//卖家名称 seller
				sendBodyDTO.setSeller(shopEr.getResult().getShopName());
			}
		}
		if(deliberyDto.getMobile()!=null&&!"".equals(deliberyDto.getMobile())){
			sendBodyDTO.setMobiletelephone(deliberyDto.getMobile());
		}
	...
	public String getDeliveryCompany(Pager<DictionaryDTO> page, Model model, String companyName, int curPage) {
		...
		return "/sellcenter/order/order_seller_deliveryCompany";
