...
public class CommonController {
	...
	public String allCategory(Model model,String type,HttpServletRequest request){
		//获取子站id
		...
		if(addressCode!=null&&!"".equals(addressCode)){
			...
			DataGrid<MallThemeDTO> dgMallTheme = mallThemeService.queryMallThemeList(theme, "1", new Pager(1,1));
			if(dgMallTheme!=null&&dgMallTheme.getRows()!=null&&dgMallTheme.getRows().size()>0){
				themeId = dgMallTheme.getRows().get(0).getId();
			}
		}
		JSONArray categoryes = this.commonService.findCategory(themeId==null?0:themeId);
