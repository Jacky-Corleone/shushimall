...
public class ShopCartController {
	...
	public ExecuteResult<String> add(HttpServletRequest request,
		...
		Long uid_ = WebUtil.getInstance().getUserId(request);
		String uid = null;
		if(uid_ != null){
	...
	public String buyAgain(HttpServletRequest request, String orderId, Model model) {
		Long uid_ = WebUtil.getInstance().getUserId(request);
		String uid = null;
		if(uid_ != null){
			uid = String.valueOf(uid_);
...
//		myCart.getShops().get(1).getProducts().get(0).getUnusualMsg().add("123333");
	...
	public ModelAndView checkShop(HttpServletRequest request, Long shopid, Boolean checked) {
		Long uid_ = WebUtil.getInstance().getUserId(request);
		String uid = null;
		if(uid_ != null){
			uid = String.valueOf(uid_);
...
//			address.put("countryname", erCountry.getResult().getName());
	...
	public String orderSubmit(HttpServletRequest request, TradeOrdersDTO dto,Model model ) {
		...
		if(uid_ != null){
			uid = String.valueOf(uid_);
			String clientuid = request.getParameter("clientuid");
			if (StringUtils.isNotEmpty(clientuid)) {
				dto.setCreatorId(Long.valueOf(clientuid));
			}
		}
		// 0询价订单 1协议订单 2正常
		...
		List<ProductInPriceDTO> products = new ArrayList<ProductInPriceDTO>();
		String promptly = "";
		UserDTO userDTO = userService.queryUserById(uid_);
		String buyNowProducts = redisDB.get("buyNowProducts_"+userDTO.getUname());
		// 立即购买或加入购物车
		...
		if(StringUtils.isNotBlank(couponId)){
			ExecuteResult<CouponsDTO> er = couponsExportService.queryById(couponId);
			if (er.isSuccess() && er.getResult() != null) {
				CouponsDTO couponsDTO = er.getResult();
...
//		 "127.0.0.1:8080/mall-web"));
    ...
    public ExecuteResult<String> promptlybuyProducts(HttpServletRequest request){
		...
		if (null != pros && pros.size() > 0) {
			for(Product product : pros){
				...
				Long parentId = WebUtil.getInstance().getUserId(request);
				UserDTO userDTO = userService.queryUserById(parentId);
				redisDB.addObject("buyNowProducts_"+userDTO.getUname(), products, 60 * 60 * 24);
				return result;
				/*
				// 获取该sku的库存
	...
	/**
	 * 
	 * <p>Discription:[提交订单，合同确认，跳转到浮层]</p>
	 * Created on 2016年3月1日
	 * @param cityCode
	 * @return
	 * @author:[化亚会]
	 */
	@RequestMapping("onSubmitForLayer")
	public String onSubmitForLayer(HttpServletRequest request, TradeOrdersDTO dto,Model model){
		String ctoken = CookieHelper.getCookieVal(request, Constants.CART_TOKEN);
		Long uid_ = WebUtil.getInstance().getUserId(request);
		String uid = null;
		if(uid_ != null){
...
//		LOG.debug("DEF ADDRESS:" + JSON.toJSONString(ckAddress));	
		...
		myCart = this.shopCartService.getMyCartToOrderView(products, ctoken, uid, false, promptly, couponId,
				paymentMethod, integral, freightDeliveryType,
				com.camelot.openplatform.common.Constants.PLATFORM_KY_ID,null,null);
		ExecuteResult<InvoiceDTO> inDTO = this.invoiceExportService.queryById(dto.getInvoiceId());
		model.addAttribute("invoiceDTO", dto); //发票信息
		model.addAttribute("tradeOrder", dto); //订单信息
		model.addAttribute("myCart", myCart);  //商品信息
		return "/shopcart/order_contract_layer";
	}
}
