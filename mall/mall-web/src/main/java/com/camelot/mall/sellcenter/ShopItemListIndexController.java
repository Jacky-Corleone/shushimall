...
public class ShopItemListIndexController {
	...
	public String toListIndex(Integer page, Long shopId, SearchInDTO itemInDTO,HttpServletRequest request,Model model){
		...
		initCoupons( shopId, request, model);
		return "/sellcenter/shop/shopProList/" + getTemplateName(shopId, model,request);
	}
	...
	public String toIndex(Integer page, Long shopId, SearchInDTO itemInDTO,HttpServletRequest request,Model model){
		...
		BaseTDKConfigDTO  baseTDKConfigDTO=this.getShopBrands(shopId);
	 	...
	 	//显示优惠券
	 	initCoupons( shopId, request, model);
		return "/sellcenter/shop/shopIndex/" + getTemplateName(shopId, model,request);
	}

	public void initCoupons(Long shopId ,HttpServletRequest request,Model model){
		...
		couponsDTO.setShopId(shopId);
		//只查询已经开始和未开始的优惠券
		List<Integer> stateList = new ArrayList<Integer>();
		stateList.add(0);
		stateList.add(1);
		couponsDTO.setStateList(stateList);
		//只查询未发放用户的优惠券,店铺优惠券领取
		...
		if(couponsDTOs != null && !couponsDTOs.isEmpty()){
			model.addAttribute("couponsDTOsIsNotNull", true);
		}else{
			model.addAttribute("couponsDTOsIsNotNull", false);
		}
	}
	...
	public BaseTDKConfigDTO getShopBrands(Long shopId){
		...
		baseTDKConfigDTO2.setTitle(shopInfo.getShopName()+" - 舒适100");

		baseTDKConfigDTO2.setDescription(shopInfo.getShopName()+","+shopInfo.getIntroduce()+"【舒适100，全国配送，优惠多多】");
	 ...
	 * @return
	 */
	private String getTemplateName(Long shopId, Model model,HttpServletRequest request) {
		...
		if(null != shopTemplesCombin){
			...
			/*ExecuteResult<ShopDTO> shopDto=shopExportService.findShopInfoById(templesDTO.getShopId());
			String uid = CookieHelper.getCookieVal(request, Constants.USER_ID);
			ExecuteResult<List<Long>> idsEr = userExportService.queryUserIds(shopDto.getResult().getSellerId());
			if(null!=renovationMap){
				Iterator it = renovationMap.entrySet().iterator(); 
				while(it.hasNext()){ 
					Map.Entry entry=(Entry) it.next();
					ShopRenovationDTO shopRenovationDTO=(ShopRenovationDTO)entry.getValue(); 
					if(null==shopRenovationDTO||null==shopRenovationDTO.getSkuId()){
						continue;
					}
					ExecuteResult<ItemDTO> res=itemExportService.getItemById(shopRenovationDTO.getSkuId());
					if(res.getResult()!=null&&!new Integer(ItemStatusEnum.SALING.getCode()).equals(res.getResult().getItemStatus())){
						if(StringUtils.isEmpty(uid)||!idsEr.getResult().contains(Long.parseLong(uid))){
							it.remove();
						}
					  }
					} 
	...
	private String initLeftPage(Long shopId, SearchInDTO itemInDTO, Model model, HttpServletRequest request) {
		...
		idlist.add(shopInfo.getSellerId());
	 ...
	 * @return
	 */
	@RequestMapping("/ajaxqueryShopProduct")
	@ResponseBody
	public Map<String,Object> ajaxqueryShopProduct(Integer page, Long shopId, ItemQueryInDTO itemInDTO, Model model){
	...
	@ResponseBody
	public Map<String, Object> loadFavourite(Long shopId, HttpServletRequest request, Model model){
		...
		if(result.isSuccess() && result.getResult() != null && result.getResult().getRows() != null){
			List<FavouriteCountDTO> fcList = result.getResult().getRows();
			for(FavouriteCountDTO favouriteCountDTO : fcList){
				//默认显示价格
				favouriteCountDTO.getItemShopCartDTO().setHasPrice(true);
				//查询商品是否暂无报价
				ExecuteResult<ItemBaseDTO> itemBaseDTOResult = this.itemExportService.getItemBaseInfoById(favouriteCountDTO.getItemShopCartDTO().getItemId());
				//暂无报价
				if(2 == itemBaseDTOResult.getResult().getHasPrice().intValue()){
					favouriteCountDTO.getItemShopCartDTO().setHasPrice(false);
				}
			}
		}
	...
	@ResponseBody
	public Map<String, Object> loadHotSales(Long shopId, HttpServletRequest request, Model model){
	...
	public String noticeView(HttpServletRequest request, Model model, @PathVariable("shopId")Long shopId, @PathVariable("id")Long id){
		...
		return "/sellcenter/shop/shopNoticeView/" + getTemplateName(shopId, model,request);
	}
	...
	public String noticeList(Integer page, HttpServletRequest request, Model model, @PathVariable("shopId")Long shopId){
		...
		return "/sellcenter/shop/shopNoticeList/" + getTemplateName(shopId, model,request);
	}
	...
	private JSONObject convertToJSONObject(CouponsDTO couponsDTO, HttpServletRequest request) {
		...
		Date date = new Date();
		//当优惠券状态为未开始或者已开始时
		 if(couponsDTO.getState() == 1 || couponsDTO.getState() ==0){
		    	// 保存用户优惠券变量
        ...
        if (registerDTO != null) {
        	...
        	ExecuteResult<DataGrid<CouponUserDTO>> result = couponsExportService.queryCouponsUserList(couponUserDTO, null);
        	if (result.isSuccess() 
    				&& result.getResult() != null 
    				&& result.getResult().getRows() != null
    				&& result.getResult().getRows().size() > 0) {
