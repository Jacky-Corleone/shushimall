...
public class PaymentExportServiceImpl extends ImplUtil implements PaymentExportService {
	...
	public ExecuteResult<Integer> payIndex(PayReqParam payReq) throws Exception{
		logger.info("\n 方法[{}]，入参：[{}]","PaymentExportServiceImpl-payIndex",JSONObject.toJSONString(payReq));
		...
		logger.info("tranQuery：[{}]", JSONObject.toJSONString(tranQuery));
		if (tranQuery == null) {			
			/******* 创建支付订单交易对象   ********/
			...
			if(TransationsStatusEnum.PAID_SUCCESS.getCode()==resultStatus){
				tranQuery = transationsDAO.selectTransByOrderNo(payReq.getOrderNo());
		...
		logger.info("\n 方法[{}]，出参：[{}]","PaymentExportServiceImpl-payIndex",JSONObject.toJSONString(result));
		return result;
	}

	@Override
	public ExecuteResult<Integer> pay(PayReqParam payReq) throws Exception{
		logger.info("\n 方法[{}]，入参：[{}]","PaymentExportServiceImpl-pay",JSONObject.toJSONString(payReq));
		// 1.必填参数是否为空
		...
		if (result.isSuccess() && TransationsStatusEnum.PAYING.getCode().equals(tranQuery.getStatus())) {
			...
			if (orderQuery.isSuccess() && orderQuery.getResult() != null) {
				...
				tranQuery.setPlatformId(tradeOrdersDTO.getPlatformId());
				int verifyAmount = tranQuery.getOrderAmount().compareTo(tradeOrdersDTO.getPaymentPrice());
				
				if(verifyAmount==0){
					
					//根据支付类型。更新支付订单的对外交易号 ，默认为901，微信101，支付宝201，网银在线301，中信401，银联801
					Transations trans = transationsDAO.selectTransByOrderNo(payReq.getOrderNo());
					String outTranNo = null;
					if (null != trans && trans.getPayBank().equals(payReq.getPayBank().name()) && !trans.getOutTradeNo().equals("0")) {
		...
		logger.info("\n 方法[{}]，出参：[{}]","PaymentExportServiceImpl-pay",JSONObject.toJSONString(result));
		return result;
	}
	...
	public ExecuteResult<OrderInfoPay> payResult(Map<String, String> params,String payBank) {
			...
			if (tranQuery != null&&!TransationsStatusEnum.USER_LESS.getCode().equals(tranQuery.getStatus())) { // 获取有效的交易记录
				if (TransationsStatusEnum.PAYING.getCode().equals(tranQuery.getStatus())) {
					...
					if(handleResult.isSuccess()){
						...
						//针对延期付款的情况，如果买家已经确认收货，但是在确认收货的时候没有生成结算单，则此时需要在付款的时候生成结算单
						//tradeOrderDTO设置支付状态为已支付，但是此处不存入数据库
						//只有企业支付支持延期退款
						//
						ExecuteResult<TradeOrdersDTO> orderQuery=tradeOrderExportService.getOrderById(tranQuery.getOrderNo());
						TradeOrdersDTO tradeOrdersDTO = orderQuery.getResult();
						if(tradeOrdersDTO.getPaymentMethod() == PayMethodEnum.PAY_ENTERPRISE.getCode() && tradeOrdersDTO.getState() !=null && tradeOrdersDTO.getState() > 3 ){
							tradeOrdersDTO.setPaid(2);
							ExecuteResult<String> re = new ExecuteResult<String>();
							try {
								List<RefundOrder> refundOrderList = new ArrayList<RefundOrder>();
								re = this.statementExportService.createSettleDetail(tradeOrdersDTO,refundOrderList);
								if(!re.isSuccess()){
									logger.error("结算单生成失败！"+re.getErrorMessages());
									result.addErrorMessage(MsgCodeEnum.info(MsgCodeEnum.Business_Logic_Error,"结算单生成失败，但支付成功，无需重新支付"));
								}
								//更新促销活动结算单中为有效状态
								ExecuteResult<ActivityStatementsDTO> ext=activityStatementSerice.queryActivityStatementsByOrderId(tradeOrdersDTO.getOrderId());//查询该订单的平台优惠总金额
		...
		if(isSuccess){
			if(PayJournayStatusEnum.SYNCHRONOUS_NOTICE.getCode().equals(notifyType)){
				...
				if(payBank.equals(BankSettleTypeEnum.AP.getLable())){
					result.setResultMessage("success");
				}else if(payBank.equals(BankSettleTypeEnum.CB.getLable())){
					result.setResultMessage("ok");
				}else if(payBank.equals(BankSettleTypeEnum.WX.getLable())){
					String res = "<xml><return_code><![CDATA[SUCCESS]]></return_code><return_msg><![CDATA[OK]]></return_msg></xml>";
					result.setResultMessage(res);
				}
			}
		}else{
	...
	private ExecuteResult<List<Transations>> handleBankResult(Transations tranQuery,Transations tranBank,String payBank){
		...
		if(!StringUtils.isEmpty(tranBank.getToAccount())){
			tranQuery.setToAccount(tranBank.getToAccount());
		}
		...
		if(PayBankEnum.CITIC.name().equals(tranQuery.getPayBank())){
			if(PayOrderTypeEnum.Parent.ordinal()==tranQuery.getPayOrderType()){ // 父单
				...
				for (CiticPayJournal item:listPayResult) {
					Transations tranItem =transationsDAO.selectTransByOutTrade(item.getOrderParentTradeNo());
...
//		transationsDAO.updateByOrderId(tranParent);
		...
		//查询父订单信息，修改父订单价格
		Transations parentTransations =transationsDAO.selectTransByOrderNo(orderQuery.getResult().getParentOrderId());
		ExecuteResult<TradeOrdersDTO> parentOrderQuery=tradeOrderExportService.getOrderById(orderQuery.getResult().getParentOrderId());
		parentTransations.setOrderAmount(parentOrderQuery.getResult().getPaymentPrice());
		transationsDAO.updateAmount(parentTransations);
		return result;
	}
	...
	// TODO 废弃代码
	public ExecuteResult<Integer> refundApply(RefundReqParam refundReqParam) {
		ExecuteResult<Integer> res = new ExecuteResult<Integer>();
		Transations transations=transationsDAO.selectTransByOrderNo(refundReqParam.getOrderNo());
		if("0".equals(transations.getOutTradeNo())){
		transations=transationsDAO.selectTransByOrderNo(transations.getOrderParentNo());
		}
		refundReqParam.setOutTradeNo(transations.getOutTradeNo());//对外交易号
		refundReqParam.setTransactionNo(transations.getTransactionNo());//返回的对外交易号
		String refundOutTradeNo=transationsDAO.selectOutTranNo(refundReqParam.getPayBank());
		refundReqParam.setOutRefundNo(refundOutTradeNo);//退款的对外交易号
		RefundTransationsDTO refundTransationsDTO=new RefundTransationsDTO();//退款记录
		refundTransationsDTO.setCodeNo(refundReqParam.getCodeNo());//退款单号
		refundTransationsDTO.setOrderNo(refundReqParam.getOrderNo());//订单号
		refundTransationsDTO.setPayBank(refundReqParam.getPayBank());//支付方式
		refundTransationsDTO.setTotalAmount(transations.getOrderAmount());//总金额
		refundReqParam.setTotalAmount(transations.getOrderAmount());//总金额
		refundTransationsDTO.setRefundAmount(refundReqParam.getRefundAmount());//退款金额
		refundTransationsDTO.setInsertTime(new Date());//操作时间
		refundTransationsDTO.setInsertBy(refundReqParam.getId());//当前操作人
		refundTransationsDTO.setOutRefundNo(refundReqParam.getOutRefundNo());//退款对外交易号
		ExecuteResult<RefundTransationsDTO> dto=refundTransationsService.createRefundTransationsDTO(refundTransationsDTO);//添加退款记录
		logger.info("\n 方法[{}]，入参：[{}]","PaymentExportServiceImpl-refundApply",JSONObject.toJSONString(refundReqParam));
		ExecuteResult<HashMap<String,String>> er = this.getPayService(refundReqParam.getPayBank()).buildRefundForm(refundReqParam);//调用退款接口
		HashMap<String,String> map=er.getResult();//退款返回结果
		String code="success";
		if(map.containsKey("resultCode")){
			 code = map.get("resultCode");//返回状态码
		}
		String message="操作成功";
		if(map.containsKey("resultMessage")){
			message = map.get("resultMessage");//返回信息
		}
		//退款url
		String url="";
		...
		res.setResultMessage(url);
		
		String state = map.get("state");//返回退款状态（1、退款成功，2、退款失败，3、申请成功，4、申请失败）
		ExecuteResult<RefundTransationsDTO> result=refundTransationsService.queryRefundTransationByRefundNo(refundReqParam.getOutRefundNo());
		if(result.getResult()!=null){
			refundTransationsDTO=result.getResult();
			refundTransationsDTO.setOutTradeNo(refundReqParam.getOutTradeNo());//对外交易号
			refundTransationsDTO.setState(Integer.parseInt(state));//状态
			refundTransationsDTO.setResultCode(code);//返回状态码
			refundTransationsDTO.setResultMessage(message);//返回信息
			refundTransationsService.updateRefundTransationsDTO(refundTransationsDTO);//更新退款记录信息
		}
		res.setResult(Integer.parseInt(state));//状态
		if("2".equals(state)||"4".equals(state)){
			res.addErrorMessage(message);//错误信息
		}
		return res;
	}
	// TODO 废弃代码
...
//			}
	...
	@Override
	public ExecuteResult<OrderInfoPay> findTransByOrderNo(String orderNo) {
		Transations tranQuery = transationsDAO.selectTransByOrderNo(orderNo);
		OrderInfoPay orderInfoPay = new OrderInfoPay();
		ExecuteResult<OrderInfoPay> er = new ExecuteResult<OrderInfoPay>();
		if (null != tranQuery) {
			orderInfoPay = this.buildOrderInfo(tranQuery, false, null);
			er.setResult(orderInfoPay);
		} else {
			List<String> errs = new ArrayList<String>();
			errs.add("不存在有效的订单记录");
			er.setErrorMessages(errs);
		}
		return er;
	}

	@Override
	public ExecuteResult<Map<String, String>> parseXml(String resultXml) {
		logger.info("\n 方法[{}]，入参：[{}][{}]","PaymentExportService-wxPayResult",resultXml);
		ExecuteResult<Map<String, String>> er = new ExecuteResult<Map<String, String>>();
		try {
			Map<String, String> params = XMLUtil.doXMLParse(resultXml);
			er.setResult(params);
		} catch (JDOMException e) {
			er.addErrorMessage("解析xml失败，失败原因："+e.getMessage());
			logger.error("解析xml失败，失败原因：",e);
		} catch (IOException e) {
			er.addErrorMessage("解析xml失败，失败原因："+e.getMessage());
			logger.error("解析xml失败，失败原因：",e);
		}
