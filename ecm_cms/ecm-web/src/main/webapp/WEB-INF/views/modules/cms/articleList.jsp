	...
	<table id="contentTable" class="table table-striped table-bordered table-condensed">
		...
		<c:forEach items="${page.list}" var="article">
			<tr>
				...
				<td>
					<a href="${ctx}/cms/article/preview?id=${article.id}" target="_blank">访问</a>
					<shiro:hasPermission name="cms:article:edit">
	    				...
	    				<shiro:hasPermission name="cms:article:audit">
	    					<a href="${ctx}/cms/article/publish?id=${article.id}" onclick="return confirmx('确认要${article.delFlag ne 0?'发布':'重新发布'}${article.delFlag ne 0?'发布':'重新发布'}该文章吗？', this.href)" >${article.delFlag ne 0?'发布':'重新发布'}</a>
	    					<c:if test="${article.delFlag ne 1}" >
	    					<a href="${ctx}/cms/article/delete?id=${article.id}&isRe=false&categoryId=${article.category.id}" onclick="return confirmx('确认要删除该文章吗？', this.href)" >删除</a>
	    					</c:if>
						</shiro:hasPermission>
					</shiro:hasPermission>
				</td>
