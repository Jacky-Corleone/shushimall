...
<head>
    ...
    <script type="text/javascript">
        ...
        function selectpage(p){
            ...
            $.ajax({
                url:"${ctx}/informationchangingaudit/selectpp/",
                type:"post",
                data:{
                    page:page,
                    rows:rows,
                ...
                success:function(data){
                    ...
                    if(data.success){
                        ...
                        $(data.obj.list).each(function(i,item){
                            num++;
                            html2=html2+"<tr><td>"+num+"</td><td>"+"新品牌申请</td><td>"+getcode(item.uid)+"</td><td>"+getcode(item.companyname)+"</td><td>"+getcode(item.sqdate)+"</td>"
                            if(item.shopid){
                                html2=html2+"<td><a href='${ctx}/informationchangingaudit/shopauditdetailpp?shopid="+getcode(item.shopid)+"&company="+getcode(item.companyname)+"&uid="+getcode(item.uid)+"'>查看申请<a></td>"
                            }else{
                                html2=html2+"<td></td>"
                            }
                            html2=html2+"</tr>";
...
<div class="content sub-content">
    <div class="content-body content-sub-body">
        ...
        <form:form id="searchForm" modelAttribute="user" action="${ctx}/informationchangingaudit/shopauditselectpp/" method="post" class="breadcrumb form-search">
            ...
            <div class="row-fluid">
                <div class="span10 div-hang">
                    ...
                    <div>
                        ...
                        <a style="margin-left:5px;" class="y-button sellerHover" href="#">新品牌申请</a>
                    </div>
                </div>
            </div>
            <div class="row-fluid">
        ...
        <table id="contentTable" class="table table-striped table-bordered table-condensed td-cen">
            ...
            <c:forEach items="${page.list}" var="user" varStatus="s">
                <tr>
                    ...
                    <td>${user.sqdate}</td>
                    <td>
                        <a href="javascript:showdetaile('${ctx}/informationchangingaudit/shopauditdetailpp?shopid=${user.shopid}&company=${user.companyname}&uid=${user.uid}')">查看申请</a>
                    </td>
                </tr>
            </c:forEach>
            </tbody>
