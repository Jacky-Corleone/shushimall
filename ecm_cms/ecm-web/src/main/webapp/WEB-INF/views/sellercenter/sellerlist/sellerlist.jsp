                ...
                success:function(data){
                    ...
                    if(data.success){
                        ...
                        $(data.obj.list).each(function(i,item){
                            ...
                            html2=html2+"<tr><td>"+getcode(item.num)+"</td><td>"+getcode(item.uid)+"</td><td>"+getcode(item.uname)+"</td><td>"+getcode(item.nickname)+"</td><td>"+getcode(item.umobile)+"</td><td>"+getcode(item.userEmail)
                                    +"</td><td>"+getcode(item.created)+"</td><td>"+getcode(item.quickType)+"</td>"
...
<div class="content sub-content">
    <div class="content-body content-sub-body">
        ...
        <form:form id="searchForm" modelAttribute="user" action="${ctx}/sellerlist/sellerlist/" method="post" class="breadcrumb form-search">
            ...
            <div class="span5">
                <label class="label-left control-label" for="useCodeid" title="用户编号">
                    用户编号：
                </label>
                <input name="uid" id="useCodeid" style="width:50%" type="text" class="form-control" onkeyup="numInput(this,0)" onbeforepaste="clipboardData.setData('text',clipboardData.getData('text').replace(/[^\d]/g,''))" maxlength="18"/>
            </div>
            <div class="span5">
                <label class="label-left control-label" for="quickTypeid" title="注册来源">
                    注册来源：
                </label>
                <select name="quickType" id="quickTypeid" style="width:50%"  class="form-control">
                    <option value="">请选择</option>
                    <option value="1">普通注册</option>
                    <option value="2">快速注册</option>
                </select>
            </div>
            </div>
            <div class="row-fluid" style="margin-top:10px;">
                <div class="span5">
                    <label class="label-left control-label" for="nickNameid" title="用户昵称">
                        用户昵称：
                    </label>
                    <input name="nickname" id="nickNameid" style="width:50%" type="text" class="form-control" />
                </div>
