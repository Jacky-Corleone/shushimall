...
<head>
    ...
    <script type="text/javascript">
        ...
        function unset(){
            ...
            $("#searchForm").attr("action","${ctx}/businesslist/citicbuller/").submit();
        }
        function page(n,s){
            $("#pageNo").val(n);
            $("#pageSize").val(s);
        ...
        function selectpage(p){
            ...
            $.ajax({
                url:"${ctx}/businesslist/selectciticbuller/",
                type:"post",
                data:{
                    page:page,
                    rows:rows,
                ...
                success:function(data){
                    ...
                    if(data.success){
                        ...
                        $(data.obj.list).each(function(i,item){
                            html2=html2+"<tr><td>"+getcode(item.num)+"</td><td>"+getcode(item.uid)+"</td><td>"+getcode(item.companyname)+"</td><td>"+getcode(item.rz)+"</td><td>"+getcode(item.rzs)+"</td><td>"+getcode(item.zf)
                                    +"</td><td>"+getcode(item.zfs)+"</td>"
...
<div class="content sub-content">
    <div class="content-body content-sub-body">
        ...
        <form:form id="searchForm" modelAttribute="user" action="${ctx}/businesslist/citicbuller/" method="post" class="breadcrumb form-search">
            <input id="pageNo" name="page" type="hidden" value="${citicbuller.page.pageNo}" />
            <input id="pageSize" name="rows" type="hidden" value="${citicbuller.page.pageSize}" />
            ...
            <div class="row-fluid">
                <div class="span5 div-hang">
                    ...
                    <a class="y-button sellerHover" href="${ctx}/businesslist/citicseller/"><i class="sellerIcon"></i>卖家</a>
                    </div>
                </div>
                <div class="span5">
...
<!-- 									<option value="2">绿印平台</option> -->
            ...
            <div class="row-fluid" style="margin-top:10px;">
                <div class="span5">
                    ...
                    <input name="registResource" id="registResourceid" style="width:50%" type="text" class="form-control" onkeyup="this.value=this.value.replace(/\D/g,'')" onafterpaste="this.value=this.value.replace(/\D/g,'')" maxlength="18" />
                </div>
                <div class="span5">
                    <label class="label-left control-label" for="userFlagid" title="公司名称">
                        公司名称：
                    </label>
            ...
            <div class="row-fluid" style="margin-top:10px;">
                <div class="span5">
                    ...
                    <select name="citicflag" id="citicflag" style="width:50%"  class="form-control">
                        ...
                        <c:forEach items="${citicbuller.zxstatus}" var="item">
        ...
        <table id="contentTable" class="table table-striped table-bordered table-condensed td-cen hhtd">
            ...
            <tr>
                ...
                <th width="20%">公司名称</th>
                <th width="17%">融资账户</th>
                <th width="10%">融资账户状态</th>
                <th width="17%">支付账户</th>
                <th width="10%">支付账户状态</th>
                <th width="10%">平台类型</th>
                <th width="10%">操作</th>
            </tr>
            </thead>
            <tbody id="tabletbody">
            <c:forEach items="${citicbuller.page.list}" var="user">
                <tr>
                    <td>${user.num}</td>
                    <td>${user.uid}</td>
...
<%--                                 <c:when test="${user.platformId==2}">绿印平台</c:when> --%>
                    ...
                    <td>
                        <c:if test="${user.uid!=''}">
                        <a href="javascript:void(0)" onclick="showmjj('${user.uid}')">查看</a>
                        </c:if>
                    </td>
                </tr>
            </c:forEach>
        ...
        <div class="pagination">${citicbuller.page}</div>
    </div>
</div>
</body>
</html>
