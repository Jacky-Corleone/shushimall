...
<head>
    ...
    <script type="text/javascript">
        function sm(){
            ...
            var url="${ctx}/informationchangingaudit/auditlmresult";
            var id=$("#id").val();
            audit(id,url,data);
        }
        function hd(){
        ...
        function openm(shopid,cid,company,uid,id){
            $("#shopid").val(shopid);
            $("#cid").val(cid);
            $("#company").val(company);
            $("#uid").val(uid);
                    ...
                    if(data.success){
                        $.jBox.prompt(data.msg, '消息', 'info', { closed: function () {
                            ...
                            if(!tablebody||tablebody.length<1){
                                parent.getTabandrefress('店铺信息变更审核','店铺新增类目审核',true);
                            }
                        } });
                    }else{
                        $.jBox.info(data.msg);
    ...
    <div class="content-body content-sub-body" style="margin-left:3%;">
        <div class="y-box">
            <div class="">
                <ul class="ul1">
                    <li class="li1">
                        <span class="gray-color">商家编号：</span>${detailLm.uid}
                    </li>
                    <li class="li1">
                        <span class="gray-color">公司名称：</span>${detailLm.company}
                    </li>
                    <li class="li1" style= "display:none">
                        <a class="y-hx" onclick="showmj('${detailLm.uid}')" href="javascript:void(0)">查看商家信息</a>
                    </li>
                </ul>
            </div>
            <table id="contentTable1" class="y-tb1 z-tbl mb20">
                ...
                <thead>
                <tr>
                    <th style="background: #fbfaf8;">新增的类目</th>
                    <th style="background: #fbfaf8;">具体类目</th>
                    <th style="background: #fbfaf8;">审核</th>
                </tr>
                </thead>
                <tbody id="tablebodyid">
                <c:forEach items="${detailLm.listMap}" var="user">
                <tr id="${user.id}">
                    <td>${user.sj}</td>
                    <td>${user.sn}</td>
                    <td>
                        <a class="y-hx" href="javascript:audit('${user.id}','${ctx}/informationchangingaudit/auditlmresult','uid=${detailLm.uid}&company=${detailLm.company}&static=2&shopid=${detailLm.shopid}&cid=${user.cid}&id=${user.id}')">通过</a>
                        <a class="y-hx" href="#" onclick="openm('${detailLm.shopid}','${user.cid}','${detailLm.company}','${detailLm.uid}','${user.id}')">驳回</a>
                    </td>
                </tr>
                </c:forEach>
                </tbody>
...
<div class="modal hide fade" id="approveDiv">
    ...
    <form name="approveForm" id="approveForm" action="${ctx}/informationchangingaudit/auditlmresult">
