    ...
    <script type="text/javascript">
        $(document).ready(function() {
            ...
            $("#btnQuery").click(function(){
                if($("#searchForm").valid()){
                    $("#searchForm").attr("action","${ctx}/apply/buyerinfo");
                    $("#searchForm").submit();
                }
...
//                    data = $.parseJSON(data);
        ...
        //打开审核认证窗口
        function showApprove(uid,platformId){
        ...
        //查看被选择人资质
        function showZZ(uid){
            var url = "${ctx}/apply/detailj?uid="+uid;
...
//            $("#detailIframe").attr("src",url);
        ...
        function approveFast(uid){
            top.$.jBox.confirm("确认要快捷认证吗？","系统提示",function(v,h,f){
                if(v == "ok"){
                    $.ajax({
...
<div class="content sub-content">
    <div class="content-body content-sub-body">
        ...
        <form name="searchForm" id="searchForm" method="post" action="${ctx}/apply/buyerinfo"  >
            ...
            <div class="container-fluid">
                <div class="row-fluid">
                    <div class="span3">
                        ...
                        <input type="text"  name="uid"  value="${buyerinfo.uid}" title="只能是数字" class="form-control input-medium"/>
                    </div>
                    <div class="span3">
                        <label class="label-control" for="companyName" title="公司名称">
                            公司名称：
                        </label>
                        <input name="companyName" id="companyName"  type="text" class="form-control input-medium" value="${buyerinfo.companyName}" />
                    </div>
                    <div class="span3">
                        ...
                        <select name="auditStatus" id="auditStatus" class="input-medium">
                            ...
                            <c:forEach items="${allstatus}" var="t">
                                <c:choose>
                                    <c:when test="${buyerinfo.auditStatus==t.code}">
                ...
                <div class="row-fluid">
                    <div class="span5">
                        ...
                        <input id="startTime" name="auditorBegin" value='${buyerinfo.auditorBegin}' type="text" class="form-control input-medium Wdate" onclick="WdatePicker({maxDate:'#F{$dp.$D(\'auditorEnd\')}',dateFmt:'yyyy-MM-dd'});" readonly="readonly">到
                        <input id="endTime" name="auditorEnd" value='${buyerinfo.auditorEnd}' type="text" class="form-control input-medium Wdate" onclick="WdatePicker({minDate:'#F{$dp.$D(\'auditorBegin\')}',dateFmt:'yyyy-MM-dd'});" readonly="readonly">
                    </div>
                    <div class="span1">
				    </div>
                    <div class="span3">
								...
								<select name="platformId" id="platformId" class="input-medium">
									<option value="">所有</option>
									<option value="0" <c:if test="${buyerinfo.platformId==0}">selected="selected"</c:if>>舒适100平台 </option>
<%-- 									<option value="2" <c:if test="${buyerinfo.platformId==2}">selected="selected"</c:if>>绿印平台 </option> --%>
								</select>
				</div>
                    <div class="span2">
...
<%--                                 <c:when test="${item.platformId==2}">绿印平台</c:when> --%>
                        ...
                        <td>
                            ...
                            <c:if test="${2==item.userstatus}">
                                <a href="javascript:void(0)" onclick="approveFast(${item.uid})">快捷认证</a>｜
                            </c:if>
