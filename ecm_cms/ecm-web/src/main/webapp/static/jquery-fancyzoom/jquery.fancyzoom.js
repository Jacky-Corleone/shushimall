...
(function($) {
		 ...
		 */
   		return this.each(function() {
	    	...
	    	oOverlay.css({
   			...
   			$this.click(function(){
   				//add by zhangzhiqing 解决图片重新上传后，查看不刷新的问题
   				if($this.is('img')){
   					if($this.attr('src') != imgTargetSrc){
   	   					imgTargetSrc = $this.attr('src');
   	   				}
   				}else{
   					if($this.attr('href') != imgTargetSrc){
	...
	function __posCenter(iWidth,iHeight){
        var h=$(window).height();
        var heightmen=window.screen.height;
        if(!heightmen){
            heightmen=640;
        }else{
            heightmen=heightmen-100;
        }
		var iLeft = ($(window).width() - iWidth) / 2 + $(window).scrollLeft();
		var iTop = (heightmen- iHeight) / 2 + $(window).scrollTop();
