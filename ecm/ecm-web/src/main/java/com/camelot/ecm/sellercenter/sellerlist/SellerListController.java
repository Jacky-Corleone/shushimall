...
public class SellerListController extends BaseController {
    ...
    public ModelAndView sellerSelect(Model model){
        ...
        if(datagrid != null && datagrid.getRows() != null && datagrid.getRows().size() > 0){
        	Map<Long, String> usernameMapToShopName = new HashMap<Long, String>();
        	model.addAttribute("usernameMapToShopName",usernameMapToShopName);
        	for(UserDTO userDTO : datagrid.getRows()){
        		...
        		if (userDTO.getUsertype() == 3){
        			...
        			ExecuteResult<DataGrid<ShopDTO>> shoper = shopExportService.findShopInfoByCondition(shopDTO, null);
        			if (shoper.isSuccess()) {
        				usernameMapToShopName.put(userId, shoper.getResult().getRows().get(0).getShopName());
        			}
        		}
        	}
        }
    ...
    public List<Map<String,Object>> zylx (List<UserDTO> list){
        ...
        if(list!=null&&list.size()>0){
            ...
            while(i.hasNext()){
        		...
        		if (ud.getUsertype() == 3){
        			ShopDTO shopDTO = new ShopDTO();
        			Long userId = ud.getUid();
        			shopDTO.setSellerId(userId);
        			ExecuteResult<DataGrid<ShopDTO>> shoper = shopExportService.findShopInfoByCondition(shopDTO, null);
        			if (shoper.isSuccess()) {
        				map.put("shopname", shoper.getResult().getRows().get(0).getShopName());
        			}
        		}
        ...
        Json json=new Json();
          ...
          //所属平台id
            ...
            if(StringUtils.isNotEmpty(usertype)){
            	userdto.setUsertype(Integer.valueOf(usertype));
            }
            //DataGrid<UserDTO> datagrid=userExportService.findUserListByCondition(userdto, UserEnums.UserType.SELLER,pager);
            DataGrid<UserDTO> datagrid=userExportService.findUserListByCondition(userdto,null,pager);
            if(datagrid != null && datagrid.getRows() != null && datagrid.getRows().size() > 0){
