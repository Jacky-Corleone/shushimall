...
<head>
	...
	<script type="text/javascript">
		$(document).ready(function() {
			...
			$("#staticpath").focus(function(){
				var name = $("#name").attr("value");
				if(name=="" || name ==null){
					return;
				} 
				var parentId = '${category.parent.id}';
				$.post("${ctx}/cms/category/getStaticPath",{"name":name,"parentId":parentId},function(resulte){
					if(resulte !=null && resulte !=""){
						resulte = resulte.replace(/\"/g, "");
						$("#staticpath").val(resulte);
					}
				});
			});
...
<body>
	...
	<form:form id="inputForm" modelAttribute="category" action="${ctx}/cms/category/save" method="post" class="form-horizontal">
		...
		<%-- <div class="control-group">
			...
			<div class="control-group">
			<label class="control-label" for="staticpath">栏目路径:</label>
			<div class="controls">
				<form:input path="staticpath" htmlEscape="false" maxlength="100" class="required"/>
			</div>
		</div>
		<div class="control-group">
