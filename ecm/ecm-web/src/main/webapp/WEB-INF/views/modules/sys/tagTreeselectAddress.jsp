...
<head>
	...
	<script type="text/javascript">
		...
		$(document).ready(function(){
			var url = "${ctx}${url}${fn:indexOf(url,'?')==-1 ? '?' : '&'}&extId=${extId}&module=${module}&t="+new Date().getTime();
			var url = "${ctx}/sys/area/treeDataAddress?&extId=&module=&t="+new Date().getTime();
		...
		var onClickNodeShowTab = function(id){
			...
			//console.info(window.parent.location.href);
			
		}
		
		// treeNode  add /update /delete 
		var appendNode = function(parentId,newNode){
			var treeObj = $.fn.zTree.getZTreeObj("tree");
			var parentNode = treeObj.getNodeByParam("id",parentId);
			console.info(newNode);
			treeObj.addNodes(parentNode,newNode);
		}
		var updateNode = function(id,newName){
			var treeObj = $.fn.zTree.getZTreeObj("tree");
