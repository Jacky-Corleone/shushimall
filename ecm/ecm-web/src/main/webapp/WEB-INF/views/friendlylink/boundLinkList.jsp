...
<script type="text/javascript">
	...
	function boundFriendlyLink(id){
		top.$.jBox.confirm("确认要绑定该链接 吗？","系统提示",function(v,h,f){
            if(v == "ok"){
            	var pageId = $("#friendlylinkPageDBId").val();
            	$.ajax({
                    url:"${ctx}/friendlylink/boundfriendlylink",
                    type:"post",
                    data:{
                    	itemId:id,
                    	pageId:pageId
                    ...
                    success:function(data){
                    	if(data.success){
                    		$.jBox.info(data.msg);
                    		location.href ="${ctx}/friendlylink/boundlink?id="+pageId;
                    	}
                    }
                });
            }
	...
	<div class="content sub-content">
		<div class="content-body content-sub-body">
			<div class="container-fluid">
			 ...
			 <legend ><span class="content-body-bg">可添加的友情链接列表</span></legend>
					...
					<tbody>
						<c:forEach items="${page.list}" var="friendlylinkItem" varStatus="s">
	                      <tr id="${friendlylinkItem.id}" >
	                          ...
	                          <td>
	                          		<a href="javascript:void(0)" onclick="javascript:boundFriendlyLink('${friendlylinkItem.id}')">绑定</a>
	                         </td> 
	                      </tr>
	                   </c:forEach>
					</tbody>
