...
<head>
	...
	<jsp:useBean id="user"  class="com.camelot.sellercenter.mallSubTab.dto.MallSubTabDTO" scope="request" ></jsp:useBean>
	...
	<script type="text/javascript">
		...
		$(document).ready(function() {
			...
			var isImg = $("#imgRadio").val();
			if(isImg == 0){//如果不是图片 那么隐藏图片div并且选择radio
				$("input[name='isImg']:eq(0)").attr("checked",'checked');
				$("#imgDiv").hide();
				$("#typeDiv").show();
			}else{
				$("input[name='isImg']:eq(1)").attr("checked",'checked'); //选择图片按钮
				$("#imgDiv").show();
				$("#typeDiv").hide();
			}
            $('.showimg').fancyzoom({
			...
			$("#btnSubmit").click(function(){
				...
				var url=picUrlImg.replace("${filePath}","");
	    		...
	    		if( iconLink_bl && logoName_bl){
	    			$.ajax({
	    				type: "post",
	        			url: "../floorNavbar/update?themeType=${themeType}",
	        			dataType:"json",
	        			data:{
	        				title: logoName,
	        				iconLink: iconLink,
	        			...
	        			success: function(data){
	        				if(data.success){
	        					...
	        					if(result){
	        						window.location.href="${ctx}/station/floorNavbar/navBarlist?msg=editSuccess&themeType=${themeType}";
	        						$.jBox.info("图标保存成功");
	            				}
			...
			$(":radio[name='isImg']").click(function(){//单选框控制图片div
				 radioValue = $("input[type='radio']:checked").val();
			    if(radioValue == "0"){
			        $("#imgDiv").hide();
			        $("#typeDiv").show();
		...
		$(document).ready(function() {
			...
			onThemeSelect('${mallRecommNavDTO.cid}','${mallRecommNavDTO.addressId}','${mallRecommNavDTO.themeId }','${mallRecommNavDTO.themeType }');
		})
	</script>
</head>
<body>
	<form:form id="inputForm" modelAttribute="mallRecommNavDTO" method="post" class="form-horizontal">
		...
		<div class="control-group">
			...
			<div class="controls">
				<label class="lbl">
					...
					<input type="hidden" value="${mallRecommNavDTO.isImg}" name="imgRadio" id="imgRadio" maxlength="15" style="width:220px;">
					<input type="text" value="${mallRecommNavDTO.title}" name="title" id="logoName" maxlength="15" style="width:220px;">
				</label>
				<label id="logoNameLabel" style='color:red'></label>
			</div>
		</div>
		<div class="control-group">
		...
		<div class="control-group">
			...
			<div class="controls">
				<form:select id="redId" path="recId">
					...
					<form:options items="${mallRec}"  itemLabel="titleDTO"  itemValue="idDTO" class="required"/>
				</form:select>
			</div>
		</div>
		...
		<%--<div id="typeDiv">
			<div class="control-group">
				...
				<div class="controls">
				<%String m=String.valueOf(request.getAttribute("nTpye")); %>
					<select id="nType" name="nType">
						<option value="1" <%="1".equals(m)?"selected":"" %>>类型一</option>
						<option value="2" <%="2".equals(m)?"selected":"" %>>类型二</option>
						<option value="3" <%="3".equals(m)?"selected":"" %>>类型三</option>
					</select>
				</div>
			</div>
		</div>--%>
		...
		<div class="control-group">
			...
			<div class="controls">
				...
				<img  src="${filePath}${mallRecommNavDTO.picSrc }" id="picUrlImg"
		...
		<div class="control-group">
            <div class="controls">
                ...
                <input id="btnCancel" class="btn" type="button" value="返 回" onclick="window.location.href='${ctx}/station/floorNavbar/navBarlist?themeType=${themeType}'"/>
            </div>
		</div>
	</form:form>
</body>
