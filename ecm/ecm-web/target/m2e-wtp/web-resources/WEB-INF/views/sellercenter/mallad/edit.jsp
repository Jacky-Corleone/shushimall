...
<script type="text/javascript">
	...
	function startUpload(fileElementId, showImg, urlInput){
		...
		if(file.size>1048576){
			alert("上传图片过大,请不要上传文件大小超过1MB的图片");
			return false;
		}
	...
	function changeImgsize(self){
        ...
        if(self.value==11){
            $("#showimgmsg").text("仅支持JPG、JPEG、PNG、BMP格式，大小不得超过1M，建议图片尺寸：200*40");
            $("#isClose").hide();
            $("#cidDiv").hide();
            $("#skuDiv").hide();
            $("#housetypeDiv").hide();
	...
	<form:form id="inputForm" modelAttribute="dto" action="${ctx}/mallAdvertise/edit" method="post" class="form-horizontal">
		...
		<div id="imgListDiv">
			...
			<div class="control-group">
				...
				<div class="controls">
					<%--<input type="text" class="z-input01" disabled="disabled">--%>
	                <span id="showimgmsg3" style="color: red">仅支持JPG、JPEG、PNG、BMP格式，大小不得超过1M，建议图片尺寸：467*380</span>
					<span class="z-upload">
						<button type="button" class="button_4 font_12 border-1 button_small">浏览图片</button>
