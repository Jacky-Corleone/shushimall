...
<body>
	...
	<form:form id="searchForm" modelAttribute="leave" action="${ctx}/oa/leave/list/task" method="post" class="breadcrumb form-search">
		...
		<div>			
			...
			<label>创建开始：</label>
			<input id="createDateStart"  name="createDateStart"  type="text" readonly="readonly" maxlength="20" class="input-small Wdate"  value="<fmt:formatDate value="${leave.createDateStart}" pattern="yyyy-MM-dd"/>" onclick="WdatePicker({maxDate:'#F{$dp.$D(\'createDateEnd\')}',dateFmt:'yyyy-MM-dd'});"/>
			<label>创建结束：</label>
			<input id="createDateEnd" name="createDateEnd" type="text" readonly="readonly" maxlength="20" class="input-small Wdate" value="<fmt:formatDate value="${leave.createDateEnd}" pattern="yyyy-MM-dd"/>" onclick="WdatePicker({minDate:'#F{$dp.$D(\'createDateStart\')}',dateFmt:'yyyy-MM-dd'});"/>
