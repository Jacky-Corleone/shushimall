	...
	$.ajax({
		...
		success: function(data){
			if(data && data.success && data.result){
				$(".js_value_added_service_div").remove();
				$.each(data.result, function(i, valueAddedItem){
					var div_ = $('<div class="fl list_div01 js_value_added_service_div"><i>增</i></div>');
					var select_ = $('<select class="js_value_added_service wid_150" style="overflow: hidden; white-space: nowrap; text-overflow: ellipsis;" onchange="addedServiceChange(this.value)"><option value="">请选择</option></select>');
					$.each(valueAddedItem.itemShopCartDTOs, function(i, itemShopCartDTO){
						var skuName = valueAddedItem.itemName;
						$.each(itemShopCartDTO.attrSales, function(i, attrSale){
							$.each(attrSale.values, function(i, value){
								skuName = skuName + "|" + value.name;
							});
						});
						skuName = skuName + "|￥" + itemShopCartDTO.skuPrice;
						$(select_).append("<option value='" + valueAddedItem.itemId + "-" + itemShopCartDTO.skuId + "'>" + skuName + "</option>");
					});
					$(div_).append($(select_));
					$(".js_value_added_service_li").append($(div_));
				});
			}
		}
	});
...
function toProOrderView(activitesDetailsId){
	...
	$.each($(".js_value_added_service option:selected"), function(i, o){
		if($(o).val()){
...
//					        $("#shoujia").html("<div class=\"mid_f_right\" id=\"itemPrice_js\"><del>￥"+skuShowPrice+"</del></div>");
						...
						$("#marketPrice_div").css("margin-left","0");
					}
					showCountDown();
				}
			}
