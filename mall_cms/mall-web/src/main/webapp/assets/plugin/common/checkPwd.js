(function($){
	...
	$.fn.checkPwdOut = function(pwd_id, pwd_sp, pwd_hint)
	{
		...
		if(pwd.length == 0){
			...
			$("#" + pwd_hint).removeClass("fa-check-circle font_7a").addClass("fa-exclamation-circle font_fe").show();
			return false;
		}
		if(pwd.length < 6){
		...
		//	[]
			...
			if(!pwdFormat.test(pwd)){
				...
				$("#" + pwd_sp).html("");
				$("#" + pwd_hint).removeClass("fa-exclamation-circle font_fe").addClass("fa-check-circle font_7a").show();
				return true;
			}
		}
	}
