...
//			return "订单号不能为空，并且必须是数字";
		...
		if(executeResult.getResult()!=null && executeResult.getResult().getRows()!=null && executeResult.getResult().getRows().size()>0){
			...
			if(null != urlResult.getRows() && !urlResult.getRows().isEmpty() && urlResult.getRows().size()>0){
				model.addAttribute("enclosureUrl", urlResult.getRows().get(0).getEnclosureUrl());
				model.addAttribute("enclosureName", urlResult.getRows().get(0).getEnclosureName());
			}
		}else{
			model.addAttribute("tradeOrder", null);
			model.addAttribute("jsonArray", null);
			model.addAttribute("enclosureUrl", null);
			model.addAttribute("enclosureName", null);
...
//      }
	...
	public ExecuteResult<String> addEnclosure(String url,String orderId,String enclosureName) {
		ExecuteResult<String> result = new ExecuteResult<String>();
		TradeOrdersEnclosureDTO dto = new TradeOrdersEnclosureDTO();
        ...
        tradeOrdersEnclosureService.addEnclosure(dto);
